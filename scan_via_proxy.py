...
class GeneTargets:
    def __init__(self, ip_l, port_l):
        ...
        self.threads = []
        self.fecth_factor = 5
        self.total_count = len(ip_l) * len(port_l)
        self.counter = 0
    ...
    def update_stat(self):
        with open(g_scan_stat, 'a') as f_h:
            rate = "%.2f%%" % (self.counter / self.total_count * 100)
            stat_str = rate + "  " + str(self.counter) + "/" + str(self.total_count) + \
...
def get_top_ports(str_port_num):
    ...
    if port_cnt <= 0:
        # defualt top 15 ports
        port_cnt = g_default_top_ports
    elif port_cnt > 8400:
...
def clt_help():
    print("A port scan tool via proxy")
...
if __name__ == '__main__':
    ...
    # ports by default
    if len(port_list) == 0:
        port_list.extend([80,443,22,3306,8443,6379,8161,8080,27017,5984,11211,2049,\
                          9200,5601,9990,5900,5901,837,50070,2181,2375,8888,389,888])
